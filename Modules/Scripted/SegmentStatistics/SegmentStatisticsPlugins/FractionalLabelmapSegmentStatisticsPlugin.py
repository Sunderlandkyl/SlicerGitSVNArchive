...
class LabelmapSegmentStatisticsPlugin(SegmentStatisticsPluginBase):
  ...
  def computeStatistics(self, segmentID):
    ...
    segFractionalLabelName = vtkSegmentationCore.vtkSegmentationConverter.GetSegmentationFractionalLabelmapRepresentationName()
    segmentLabelmap = segment.GetRepresentation(segFractionalLabelName)
    ...
    if "voxel_count" in requestedKeys:
      stats["voxel_count"] = stat.GetFractionalVoxelCount()
    if "volume_mm3" in requestedKeys:
      stats["volume_mm3"] = stat.GetFractionalVoxelCount() * cubicMMPerVoxel
    if "volume_cm3" in requestedKeys:
      stats["volume_cm3"] = stat.GetFractionalVoxelCount() * cubicMMPerVoxel * ccPerCubicMM
    return stats
