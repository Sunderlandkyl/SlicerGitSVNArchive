...
std::string vtkSegmentation::DetermineCommonLabelmapGeometry(int extentComputationMode, const std::vector<std::string>& segmentIDs/*=std::vector<std::string>()*/)
{
  ...
  bool masterRepresentationIsFractionalLabelmap = this->GetMasterRepresentationName() == vtkSegmentationConverter::GetSegmentationFractionalLabelmapRepresentationName();
  ...
  for (std::vector<std::string>::iterator segmentIt = mergedSegmentIDs.begin(); segmentIt != mergedSegmentIDs.end(); ++segmentIt)
    ...
    vtkOrientedImageData* currentLabelmap = vtkOrientedImageData::SafeDownCast(
      masterRepresentationIsFractionalLabelmap ?
        currentSegment->GetRepresentation(vtkSegmentationConverter::GetSegmentationFractionalLabelmapRepresentationName()) :
        currentSegment->GetRepresentation(vtkSegmentationConverter::GetSegmentationBinaryLabelmapRepresentationName()));
    if (currentLabelmap==NULL || currentLabelmap->IsEmpty())
      {
      continue;
      }
    ...
    if (validExtent && currentLabelmapExtent[0] <= currentLabelmapExtent[1]
      ...
      vtkOrientedImageDataResample::GetTransformBetweenOrientedImages(currentLabelmap, commonGeometryImage, currentLabelmapToCommonGeometryImageTransform.GetPointer());
      int currentLabelmapExtentInCommonGeometryImageFrame[6] = { 0, -1, 0, -1, 0, -1 };
      vtkOrientedImageDataResample::TransformExtent(currentLabelmapExtent, currentLabelmapToCommonGeometryImageTransform.GetPointer(), currentLabelmapExtentInCommonGeometryImageFrame);
